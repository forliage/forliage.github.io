// Build:
//   nvcc -O3 -std=c++17 -arch=sm_89 main.cu -o bunny_glass \
//       -lglfw -lGL -lGLEW -ldl -lpthread
// Run:
//   ./bunny_glass stanford-bunny.obj
//
// Controls:
//   Left drag   : look around
//   W A S D     : move around
//   Mouse wheel : move forward/back
//   Click swatch: switch glass tint
...
static const TintPreset kPresets[kPresetCount] = {
    {"Clear",  Vec3(0.00f, 0.00f, 0.00f)},
    {"Amber",  Vec3(0.10f, 0.45f, 1.20f)},
    {"Green",  Vec3(1.10f, 0.12f, 1.05f)},
    {"Blue",   Vec3(1.20f, 0.80f, 0.12f)},
    {"Cyan",   Vec3(1.05f, 0.15f, 0.12f)},
    {"Red",    Vec3(0.10f, 1.20f, 1.20f)},
    {"Purple", Vec3(0.38f, 1.05f, 0.12f)},
};

__constant__ float3 c_preset_sigma[kPresetCount];

__host__ __device__ inline Vec3 preset_display_color(const Vec3& sigma) {
    Vec3 T(expf(-sigma.x * 1.0f), expf(-sigma.y * 1.0f), expf(-sigma.z * 1.0f));
    return clamp01(Vec3(powf(T.x, 1.0f / 2.2f), powf(T.y, 1.0f / 2.2f), powf(T.z, 1.0f / 2.2f)));
}
                              ...
                              RenderParams rp)
{
    ...
    for (int s = 0; s < rp.spp_per_frame; s++) {
        ...
        for (int bounce = 0; bounce < rp.max_bounces; bounce++) {
            ...
            // Beer-Lambert while traveling in glass
            if (inside_glass) {
                Vec3 att = exp3(Vec3(-rp.sigma_a.x * hit.t,
                                     -rp.sigma_a.y * hit.t,
                                     -rp.sigma_a.z * hit.t));
                throughput = mul(throughput, att);
            }
            ...
            if (hit.type == HIT_GROUND) {
                // Next-event estimation on ground with MIS
                {
                    Vec3 ln;
                    float pdfA = 0.0f;
...
int main(int argc, char** argv) {
    ...
    GLFWwindow* win = glfwCreateWindow(W, H, "CUDA Glass Bunny (GLFW)", nullptr, nullptr);
    if (!win) {
    ...
    {
        float3 sigmas[kPresetCount];
        for (int i = 0; i < kPresetCount; i++) {
            sigmas[i] = make_float3(kPresets[i].sigma_a.x, kPresets[i].sigma_a.y, kPresets[i].sigma_a.z);
        }
        CUDA_CHECK(cudaMemcpyToSymbol(c_preset_sigma, sigmas, sizeof(sigmas)));
    }
