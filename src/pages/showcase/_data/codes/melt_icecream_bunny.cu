...
//   nvcc -O3 -std=c++17 -arch=sm_89 melt_bunny_cuda.cu -o melt_bunny_cuda -lglfw -ldl -lpthread -lm
//
// Run:
//   ./melt_bunny_cuda stanford-bunny.obj 300 melt_frames
//
// Notes:
    ...
    float floor_y)
{
    ...
    if (hit) {
        ...
        Vec3 light = normalize(Vec3(0.6f, 1.2f, 0.8f));
        float diff = fmaxf(dot(n, light), 0.0f);
        Vec3 h = normalize(light - dir);
        float spec = powf(fmaxf(dot(n, h), 0.0f), 36.0f) * 0.35f;
        ...
        Vec3 cold(0.94f, 0.92f, 0.88f);
        Vec3 hot(0.98f, 0.84f, 0.71f);
        Vec3 base = lerp3(cold, hot, m);

        float fres = powf(1.0f - fmaxf(dot(n, -1.0f * dir), 0.0f), 5.0f);
        col = base * (0.18f + 0.82f * diff) + Vec3(spec, spec, spec) + Vec3(0.08f, 0.10f, 0.12f) * fres;
    } else {
        ...
        if (dir.y < -1e-4f) {
            ...
            if (tg > 0.0f && tg < t_max) {
                ...
                if (gp.x >= 0.0f && gp.x <= 1.0f && gp.z >= 0.0f && gp.z <= 1.0f) {
                    ...
                    Vec3 g0(0.80f, 0.82f, 0.84f);
                    Vec3 g1(0.72f, 0.74f, 0.76f);
...
int main(int argc, char** argv) {
    ...
    std::string out_dir = (argc >= 4) ? argv[3] : "melt_frames";
    ...
    // Material parameters (ice-cream-like bunny).
    const float ambient_temp = -8.0f;
    const float initial_temp = -10.0f;
    const float melt_center = -1.0f;
    const float melt_half = 2.5f;
    const float visc_cold = 12000.0f;
    const float visc_hot = 0.12f;

    const float thermal_kappa = 0.0009f;
    const int temp_jacobi_iters = 20;

    const int vel_diffuse_iters = 42;
    ...
    const float heat_radius = 0.17f;
    const float heat_temp = 26.0f;
    const float heat_gain = 7.5f;
    const float cool_rate = 0.20f;
    ...
    if (glfwInit()) {
        ...
        win = glfwCreateWindow(640, 360, "CUDA Melt Bunny (no OpenGL interop)", nullptr, nullptr);
        if (!win) {
    ...
    std::printf("[Run] Frames: %d, substeps/frame: %d, dt: %.6f\n", total_frames, substeps, dt);
    ...
    for (int frame = 0; frame < total_frames; ++frame) {
        if (win) {
            ...
            char title[256];
            std::snprintf(title, sizeof(title), "CUDA Melt Bunny - frame %d/%d", frame + 1, total_frames);
